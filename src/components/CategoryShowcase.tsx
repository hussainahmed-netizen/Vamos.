...
export const CategoryShowcase: React.FC = () => {
  const { navigateToCategory, categories, products } = useStore();
  ...
  return (
    <section className="max-w-[1536px] mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {categories.map((cat) => {
          const countInStore = products.filter((p) => p.category === cat.id).length;
          const displayCount = countInStore > 0 ? countInStore : (cat.itemCount > 0 ? cat.itemCount : 12);
